...
void action()
{
  ...
  HObject  ho_ImagePart, ho_Image2, ho_ResizedImage2, ho_ImagePart2;

  // Local control variables
  HTuple  hv_ModelPath, hv_PreprocessPath, hv_PreprocessParamFileName;
  ...
  HTuple  hv_ClassIDs, hv_WindowHandleDict, hv_DLSampleInference;
  HTuple  hv_DLResult, hv_detect_class_ids, hv_check_plate;
  HTuple  hv_bboxcol, hv_bboxrow, hv_bboxcol2, hv_bboxrow2;
  HTuple  hv_i, hv_index, hv_I, hv_checkid, hv_row, hv_col;
  ...
  hv_PreprocessPath = "/home/dika/Pictures/PKM_DATASET512x320";
  //Model Path
  hv_PreprocessParamFileName = hv_PreprocessPath+"/dl_preprocess_param.hdict";
  hv_TrainedModelFileName = hv_ModelPath+"/final_dl_model_detection.hdl";

  //Batch size used during inference.
  hv_BatchSizeInference = 2;

  //Postprocessing parameters for the detection model.
  hv_MinConfidence = 0.55;
  hv_MaxOverlap = 0.2;
  hv_MaxOverlapClassAgnostic = 0.2;
  ...
  ReadImage(&ho_Image, "/home/dika/Downloads/datasheetPKM/image1");
  ...
  hv_detect_class_ids = hv_DLResult.TupleGetDictTuple("bbox_class_id");
  hv_check_plate = hv_detect_class_ids.TupleEqualElem(2);
  ...
  {
  HTuple end_val76 = (hv_check_plate.TupleLength())-1;
  HTuple step_val76 = 1;
  for (hv_i=0; hv_i.Continue(end_val76, step_val76); hv_i += step_val76)
  {
    if (0 != (int(HTuple(hv_check_plate[hv_i])==1)))
    {
      hv_index = hv_i;
      GenRectangle1(&ho_Rectangle, HTuple(hv_bboxrow[hv_index]).TupleAbs(), HTuple(hv_bboxcol[hv_index]).TupleAbs(), 
          HTuple(hv_bboxrow2[hv_index]).TupleAbs(), HTuple(hv_bboxcol2[hv_index]).TupleAbs());
  ...
  for (hv_I=2; hv_I<=1200; hv_I+=1)
  {
    ReadImage(&ho_Image2, "/home/dika/Downloads/datasheetPKM/image"+(hv_I.TupleString(".0f")));

    ZoomImageSize(ho_Image2, &ho_ResizedImage2, 512, 320, "constant");

    //detect plate
    gen_dl_samples_from_images(ho_ResizedImage2, &hv_DLSampleInference);
    preprocess_dl_samples(hv_DLSampleInference, hv_DLPreprocessParam);
    ApplyDlModel(hv_DLModelHandle, hv_DLSampleInference, HTuple(), &hv_DLResult);
    dev_display_dl_data(hv_DLSampleInference, hv_DLResult, hv_DLDataInfo, "bbox_result", 
        HTuple(), hv_WindowHandleDict);
    ...
    {
    HTuple end_val109 = (hv_check_plate.TupleLength())-1;
    ...
    for (hv_i=0; hv_i.Continue(end_val109, step_val109); hv_i += step_val109)
    {
      if (0 != (int(HTuple(hv_check_plate[hv_i])==1)))
      {
        ...
        hv_index = hv_i;
        hv_row = HTuple(hv_bboxrow[hv_index]).TupleAbs();
        hv_col = HTuple(hv_bboxcol[hv_index]).TupleAbs();
        hv_row2 = HTuple(hv_bboxrow2[hv_index]).TupleAbs();
        hv_col2 = HTuple(hv_bboxcol2[hv_index]).TupleAbs();
        GenRectangle1(&ho_Rectangle, hv_row, hv_col, hv_row2, hv_col2);
        ReduceDomain(ho_ResizedImage2, ho_Rectangle, &ho_ImagePart2);

        //crop_part (ResizedImage2, ImagePart2, abs(bboxrow[index]), abs(bboxcol[index]), abs(bboxrow2[index]), abs(bboxcol2[index]))
    ...
    CopyObj(ho_Image2, &ho_Image, 1, 1);
